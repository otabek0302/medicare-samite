...
export default function UpdateCityModel({ isOpen, onClose, data }) {
  ...
  const updateRole = async (Inputdata) => {
    ...
    try {
      ...
      setisLoading(false);
      if (res.response === 200) {
        ShowToast(toast, 'success', t('transactions.update.messages.success'));
      ...
      <ModalContent
        ...
        <ModalBody>
          <Box pb={3}>
            <FormControl isRequired>
              <FormLabel>{t('transactions.update.form.title')}</FormLabel>
              <Input
        ...
        <ModalFooter py={3}>
          <Button
            ...
            onClick={onClose}
            size={'sm'}>
            {t('transactions.update.buttons.close')}
          </Button>
          <Button
            variant="solid"
            size={'sm'}
            ...
            type="submit"
            isLoading={isLoading}>
            {t('transactions.update.buttons.update')}
          </Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
