...
// ----
  const [selectedCountry, setSelectedCountry] = useState(countryOptions[0]); //JSON.parse(('selectedCountryPhone'))
  ...
  const handlePhoneChange = (input) => {
    console.log(input);
    setPhoneNumber(formatPhoneNumber(input, selectedCountry));
  };
          ...
          <CardBody p={3} as={"form"} onSubmit={handleSubmit(AddNew)}>
            <Flex gap={10}>
              <FormControl isRequired>
                ...
                <Input w={250} type="email" placeholder={t("doctors.add.form.email")} {...register("email", { required: true })} />
              </FormControl>

              <FormControl isRequired>
                <FormLabel>{t("doctors.add.form.password")}</FormLabel>
                <Input type="password" placeholder={t("doctors.add.form.password")} {...register("password", { required: true })} />
              </FormControl>
              <FormControl isRequired>
                <FormLabel>{t("doctors.add.form.confirmPassword")}</FormLabel>
                <Input type="password" placeholder={t("doctors.add.form.confirmPassword")} {...register("cnfPassword", { required: true })} />
              </FormControl>
            </Flex>
            <Flex gap={10} mt={5}>
              <FormControl isRequired>
                <FormLabel>{t("doctors.add.form.firstName")}</FormLabel>
                <Input placeholder={t("doctors.add.form.firstName")} {...register("f_name", { required: true })} />
              </FormControl>

              <FormControl isRequired>
                <FormLabel>{t("doctors.add.form.lastName")}</FormLabel>
                <Input placeholder={t("doctors.add.form.lastName")} {...register("l_name", { required: true })} />
              </FormControl>
            </Flex>
            ...
            {/* phone number validation */}
              ...
              <FormControl isRequired>
                ...
                <Input max={todayDate()} placeholder={t("doctors.add.form.dateOfBirth")} size="md" type="date" {...register("dob", { required: true })} />
              </FormControl>
            </Flex>
            ...
            <Flex gap={10} mt={5}>
              <FormControl isRequired>
                ...
                <Select placeholder={t("doctors.add.form.gender.select")} {...register("gender", { required: true })}>
                  <option value="Female">{t("doctors.add.form.gender.female")}</option> <option value="Male">{t("doctors.add.form.gender.male")}</option>
                </Select>
              </FormControl>

              <FormControl isRequired>
                <FormLabel>{t("doctors.add.form.experience")}</FormLabel>
                <Input type="number" placeholder={t("doctors.add.form.experience")} {...register("ex_year", { required: true })} />
              </FormControl>
            </Flex>
            ...
            <Flex gap={10} mt={5}>
              <FormControl isRequired>
                ...
                <ComboboxDemo name={"Department"} data={departmentList} setState={setdepartmentID} />
              </FormControl>

              <FormControl isRequired>
                <FormLabel>{t("doctors.add.form.specialization")}</FormLabel>
                <MultiTagInput data={specializationList} setState={setspecializationID} name={"Specialization"} />
              </FormControl>
            </Flex>

            <Button w={"100%"} mt={10} type="submit" colorScheme="green" size={"sm"} isLoading={isLoading}>
              {t("doctors.add.form.submit")}
            </Button>
          </CardBody>
        </Card>
        <Card mt={5} bg={useColorModeValue("white", "gray.700")} w={"25%"} h={"fit-content"} pb={10}>
          <CardBody p={2}>
            ...
            <Flex p={2} justify={"center"} mt={5} position={"relative"}>
              <Image
                ...
                objectFit={"cover"}
                w={200}
                src={profilePicture ? URL.createObjectURL(profilePicture) : profileImg}
                fallbackSrc={profileImg} 
              />
              {profilePicture && (
                <Tooltip label={t("doctors.add.profilePicture.clear")} fontSize="md">
                  <CloseButton
                    colorScheme="red"
                    variant={"solid"}
                    position={"absolute"}
